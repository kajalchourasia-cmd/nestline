#!/usr/bin/env python3
"""Verify OpenAI API/model access without making a generation or embedding call."""
...
def _fetch_model_ids(api_key: str, timeout_seconds: float) -> set[str]:
    ...
    try:
        ...
        raise RuntimeError(f"OpenAI models endpoint returned HTTP {exc.code}") from None
    except (URLError, TimeoutError, OSError, UnicodeDecodeError, json.JSONDecodeError):
        raise RuntimeError("OpenAI models endpoint was unavailable or returned invalid JSON") from None
    data = payload.get("data") if isinstance(payload, dict) else None
    if not isinstance(data, list):
        raise RuntimeError("OpenAI models response had an invalid shape")
    return {
...
def main() -> int:
    ...
    selected_model = os.environ.get("NESTLINE_OPENAI_MODEL", os.environ.get("NESTLINE_GENERATION_MODEL", "")).strip()
    report = {
        "schema_version": "nestline-openai-access-check-v1",
    ...
    if not key:
        ...
        report["error"] = "OPENAI_API_KEY is missing"
        print(json.dumps(report, indent=2, sort_keys=True))
        return 1
    try:
    ...
    print(json.dumps(report, indent=2, sort_keys=True))
    return 0 if not selected_model or selected_model in model_ids else 1
